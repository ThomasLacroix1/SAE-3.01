/** @type {import('tailwindcss').Config} */
module.exports = {
  content: ["./src/**/*.{html,js}"],
  theme: {
    extend: {
      boxShadow: {
        'card': '6px 6px 26px -2px rgba(0,0,0,0.51)',
      },
      colors:{
        "base-white": "hsl(var(--base--white))",
        "base-black": "hsl(var(--base--black))",
        "clr-light-secondary": "hsl(var(--colors--light-grey))",
        "clr-secondary": "hsl(var(--colors--grey))",
        "clr-dark-secondary": "hsl(var(--colors--dark-grey))",
        "clr-light-primary": "hsl(var(--colors--light-green))",
        "clr-primary": "hsl(var(--colors--green))",
        "clr-cons-xbox": "hsl(var(--consoles--green-xbox))",
        "clr-cons-ps": "hsl(var--consoles--blue-ps))",
        "clr-cons-nintendo": "hsl(var(--consoles--red-nintendo))",
    },
    fontFamily:{
        'inter': ["'Inter', sans-serif"],
      
  },
  spacing: {
    '256': '42rem',
  },
    },
  },
  plugins: [],
}

